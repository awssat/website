...
<div class="w-full overflow-hidden gradient-mesh">
    ...
    <section class="relative min-h-screen flex items-center justify-center overflow-hidden px-4">
        ...
        <div class="absolute inset-0 overflow-hidden pointer-events-none">
            <div class="absolute top-20 right-10 w-72 h-72 bg-primary-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float"></div>
            <div class="absolute top-40 left-20 w-96 h-96 bg-accent-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float-slow"></div>
            <div class="absolute -bottom-32 right-1/3 w-80 h-80 bg-primary-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float-fast"></div>
        </div>
            ...
            <div class="animate-on-scroll delay-400">
                <a href="{{ url('/ar/portfolio') }}"
                   ...
                   @mouseleave="handleMouseLeave">
                    {{ $page->trans('home.cta') }}
                    <svg class="inline-block w-5 h-5 mr-2 rtl:mirror" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <section class="py-24 px-4 bg-white dark-mode:bg-gray-900">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                @if(isset($page->portfolio_ar))
                    @foreach($page->portfolio_ar->where('featured', true)->take(6) as $item)
            ...
            <div class="text-center mt-12 animate-on-scroll">
                <a href="{{ url('/ar/portfolio') }}" class="inline-flex items-center px-6 py-3 rounded-xl font-semibold text-primary-600 dark-mode:text-primary-400 hover:text-primary-700 dark-mode:hover:text-primary-300 transition-colors">
                    {{ $page->trans('common.view_all') }}
                    <svg class="w-5 h-5 mr-2 rtl:mirror" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"></path>
                    </svg>
                </a>
            </div>
    ...
    {{-- Recent Blog Posts --}}
    @if(isset($page->posts_ar) && $page->posts_ar->count() > 0)
    <section class="py-24 px-4 bg-gray-50 dark-mode:bg-gray-950">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($page->posts_ar->take(3) as $post)
                    <article class="bg-white dark-mode:bg-gray-900 rounded-2xl overflow-hidden shadow-lg hover-lift animate-on-scroll">
                        <div class="p-6">
                            ...
                            <a href="{{ $post->getUrl() }}" class="inline-flex items-center text-sm font-semibold text-primary-600 dark-mode:text-primary-400 hover:text-primary-700 dark-mode:hover:text-primary-300 transition-colors">
                                ...
                                <svg class="w-4 h-4 mr-1 rtl:mirror" fill="none" stroke="currentColor" viewBox="0 0 24 24">
