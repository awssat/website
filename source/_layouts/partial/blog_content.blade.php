...
<div class="mb-12 md:mb-16 scroll-reveal px-4 sm:px-0">
    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary-100/50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 text-xs sm:text-sm font-semibold mb-4 sm:mb-6">
        <span>{{ $page->trans('blog.the_blog') }}</span>
    </div>
    <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 dark:text-white mb-4 sm:mb-6 tracking-tight text-balance">
        {{ $page->trans('blog.hero_title') }} <span class="text-transparent bg-clip-text bg-gradient-to-r from-primary-600 to-accent-500">{{ $page->trans('blog.hero_highlight') }}</span>
    </h1>
    <p class="text-base sm:text-lg md:text-xl text-gray-600 dark:text-gray-400 max-w-2xl leading-relaxed">
        {{ $page->trans('blog.hero_description') }}
    </p>
</div>
...
@if($pagination->currentPage == 1 && $items->count() > 0)
        ...
        <article class="group relative grid md:grid-cols-2 gap-6 md:gap-8 bg-white dark:bg-gray-900 rounded-2xl md:rounded-3xl p-6 md:p-8 shadow-xl hover:shadow-2xl border border-gray-200 dark:border-gray-800 transition-all duration-300 overflow-hidden">
            ...
            <div class="flex flex-col justify-center relative z-10">
                ...
                <h2 class="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-3 md:mb-4 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors leading-tight">
                    <a href="{{ $page->getPostUrl($featured) }}">
...
{{-- Recent Posts Grid --}}
<div class="mb-12">
    <h3 class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-6">{{ $page->trans('blog.recent_articles') }}</h3>
...
<nav class="flex justify-center my-16 scroll-reveal">
    <div class="inline-flex items-center p-1 rounded-full bg-white dark:bg-gray-900 shadow-xl border border-gray-200 dark:border-gray-800">
        ...
        <a href="{{ $previous }}"
           ...
           title="{{ $page->trans('blog.previous_page') }}">
            <svg class="w-5 h-5 {{ $page->locale === 'ar' ? 'rotate-180' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="{{ $next }}"
           class="w-10 h-10 flex items-center justify-center rounded-full text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-primary-600 transition-all duration-200"
           title="{{ $page->trans('blog.next_page') }}">
            <svg class="w-5 h-5 {{ $page->locale === 'ar' ? 'rotate-180' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
