<nav role="navigation" @unless($isHome ?? false) @click.away="open = false" :class="{ 'hidden': !open, 'flex': open }"
class="md:block justify-end relative z-20" @else class="flex justify-center" @endunless>
<ul
    class="list-none shadow-sm border base-side-menu-ul rounded-sm inline-flex md:flex {{ $isHome ?? false ? 'p-5' : 'md:items-center absolute md:relative mt-2 mr-1 md:mr-0' }} md:flex-row flex-col md:justify-end w-auto md:w-full">

    <li class="ml-2 @if ($page->isPath('blog*')) md:hidden @endif">
        <a href="{{ url('/blog') }}"
    ...
    <li class="ml-2 @if ($page->isPath('opensource')) md:hidden @endif">
        <a href="{{ url('/opensource') }}"
    ...
    <li class="ml-2 @if ($page->isPath('projects*')) md:hidden @endif">
        <a href="{{ url('/projects') }} "
            class="base-side-menu-li flex items-center py-2 pl-1 pr-3 md:pr-5 group">
    ...
    @unless($isHome ?? false)
    <li class="@unless($isHome ?? false) lg:absolute lg:top-0 lg:left-0 lg:-ml-16 lg:mt-2 @else fixed top-0 right-0 m-3 @endunless flex justify-center m-1"
