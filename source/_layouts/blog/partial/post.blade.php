...
<div class="relative w-full mb-12 -mt-8">
    ...
    <div class="relative w-full">
        ...
        <div class="absolute bottom-3 sm:bottom-4 md:bottom-6 right-3 sm:right-4 md:right-6">
            <div class="inline-flex items-center gap-1.5 sm:gap-2 md:gap-2.5 px-3 sm:px-4 md:px-5 py-2 sm:py-2.5 md:py-3 rounded-xl sm:rounded-2xl bg-white dark:bg-gray-900 shadow-2xl border border-gray-200/50 dark:border-gray-700/50">
...
<div class="p-4 sm:p-6 md:p-8 lg:p-12">
        ...
        <div class="flex flex-wrap items-center gap-3 md:gap-4 text-xs sm:text-sm mb-4 md:mb-6 pb-4 md:pb-6 border-b border-gray-200 dark:border-gray-800">
            ...
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    ...
                    @endif
                    <span class="text-xs text-gray-500 dark:text-gray-400">{{ $page->trans('blog.author') }}</span>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                ...
                <span>{{ $page->trans('blog.published_on') }} {{ $page->getDate()->diffForHumans() }}</span>
            </div>
            ...
            <div class="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                ...
                <span>{{ $page->getReadingTime() }} {{ $page->trans('blog.min_read') }}</span>
            </div>
            ...
            <div class="flex items-center gap-2 text-accent-600 dark:text-accent-400">
                ...
                <span>{{ $page->trans('blog.updated_on') }} {{ $page->getUpdatedAt()->diffForHumans() }}</span>
            </div>
            @endif
        </div>
    ...
    <nav class="mb-8 p-6 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700" x-data="{ tocOpen: true }">
        <button @click="tocOpen = !tocOpen" class="flex items-center justify-between w-full text-left mb-4">
            <h2 class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                {{ $page->trans('blog.table_of_contents') }}
            </h2>
    ...
    <section class="mt-12 pt-12 border-t border-gray-200 dark:border-gray-800">
        <h2 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
            ...
            {{ $page->trans('blog.related_posts') }}
        </h2>

        <div class="grid md:grid-cols-{{ $relatedPosts->count() === 2 ? '2' : '3' }} gap-6">
            @foreach($relatedPosts as $post)
            <a href="{{ $page->getPostUrl($post) }}" class="group relative bg-gray-50 dark:bg-gray-800 rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-200 dark:border-gray-700">
                {{-- Cover Image --}}
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center gap-2 mb-2 text-xs">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-md bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 font-semibold">
                            {{ $post->similarityScore }} {{ $page->trans($post->similarityScore === 1 ? 'blog.shared_tag' : 'blog.shared_tags') }}
                        </span>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
            <a href="https://github.com/awssat/website/tree/master/source/_posts/{{ $page->getFilename() }}.md"
               ...
               class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg font-medium text-sm transition-all hover:scale-105">
                ...
                {{ $page->trans('blog.improve_on_github') }}
            </a>
        </div>
    </footer>
</div>
