...
<div class="w-full min-h-screen py-24 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 flex flex-col {{ $page->locale === 'ar' ? 'lg:flex-row-reverse' : 'lg:flex-row' }} gap-12 relative z-10">
        {{-- Main Content --}}
        ...
        <aside class="w-full lg:w-1/4">
            <div class="lg:sticky lg:top-4 space-y-6">
                ...
                <div class="relative bg-gradient-to-br from-white via-white to-primary-50/30 dark:from-gray-900 dark:via-gray-900 dark:to-primary-900/20 rounded-3xl p-6 shadow-xl border border-gray-200 dark:border-gray-800 scroll-reveal overflow-hidden">
                    ...
                    <div class="relative">
                        <div class="flex items-center {{ $page->locale === 'ar' ? 'space-x-reverse space-x-3' : 'space-x-3' }} mb-4">
                            <div class="relative">
                                <img src="/assets/images/logo.png" class="w-12 h-12 transition-transform hover:rotate-12 duration-300" alt="Awssat Logo">
                                <div class="absolute -bottom-1 {{ $page->locale === 'ar' ? '-left-1' : '-right-1' }} w-4 h-4 bg-green-500 rounded-full border-2 border-white dark:border-gray-900 animate-pulse"></div>
                            </div>
                            <div>
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-400 leading-relaxed mb-4">
                            {{ $page->trans('blog.sidebar_description') }}
                        </p>
                        ...
                        <div class="grid grid-cols-2 gap-3 pt-4 border-t border-gray-200 dark:border-gray-800">
                            <div class="text-center p-2 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                                ...
                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ $page->trans('blog.github_stars') }}</div>
                            </div>
                            <div class="text-center p-2 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                                <div class="text-lg font-bold text-accent-600 dark:text-accent-400">6</div>
                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ $page->trans('blog.laravel_prs') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <a href="{{ $page->getTagUrl($tag->filename) }}"
                           ...
                           class="group flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200
                            ...
                            <svg class="w-4 h-4 transition-transform {{ $page->locale === 'ar' ? 'rotate-180 group-hover:-translate-x-1' : 'group-hover:translate-x-1' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="/blog/feed.atom"
                   class="group relative flex items-center justify-center p-5 rounded-2xl bg-gradient-to-br from-orange-500/10 via-orange-500/5 to-red-500/10 border border-orange-200/50 dark:border-orange-900/30 text-orange-600 dark:text-orange-400 hover:scale-[1.02] hover:shadow-xl transition-all duration-300 overflow-hidden">
                    {{-- Animated Background --}}
                    <div class="absolute inset-0 bg-gradient-to-r from-orange-500/0 via-orange-500/10 to-orange-500/0 {{ $page->locale === 'ar' ? 'translate-x-[100%] group-hover:translate-x-[-100%]' : 'translate-x-[-100%] group-hover:translate-x-[100%]' }} transition-transform duration-1000"></div>
                    ...
                    <div class="relative flex items-center gap-3">
                        ...
                        <div class="{{ $page->locale === 'ar' ? 'text-right' : 'text-left' }}">
                            <div class="text-sm font-bold">{{ $page->trans('blog.subscribe_rss') }}</div>
                            <div class="text-xs text-orange-500/80 dark:text-orange-400/80">{{ $page->trans('blog.never_miss_update') }}</div>
                        </div>
                    </div>
                </a>
            </div>
