...
<article class="group relative bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-lg hover-lift border border-gray-200 dark:border-gray-800 transition-all duration-300">
    {{-- Type Badge --}}
    <div class="absolute top-4 {{ $page->locale === 'ar' ? 'left-4' : 'right-4' }} z-10">
        @if($item->type === 'laravel-pr')
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-primary-100 text-primary-700 dark:bg-primary-900 dark:text-primary-300">
        ...
        @else
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-accent-100 text-accent-700 dark:bg-accent-900 dark:text-accent-300">
                <svg class="w-3 h-3 {{ $page->locale === 'ar' ? 'ml-1' : 'mr-1' }}" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="p-6">
        ...
        <div class="flex flex-wrap gap-2 mb-4">
            @foreach($item->tech_stack as $tech)
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300">
                    {{ $tech }}
                </span>
            @endforeach
        </div>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-800">
            ...
            {{-- View Link --}}
            <a href="{{ $item->getUrl() }}" class="inline-flex items-center text-sm font-semibold text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 transition-colors">
